...
class TournamentEmpty:
    """
    Definition d'un tournoi non peuplé: 
    nom, lieu, date de debut, date de fin, notes  
    """
...
class Tournament(TournamentEmpty):
    """
    ...
    def __init__(self, name: str, location: str, start_date: datetime.date, end_date: datetime.date, nb_round: int ,num_round: int , players: List[Player], stage: List[Round], ranking: List[int], notes: str = ""):
        super().__init__(name, location, start_date, end_date, notes)
        nb_round = 0
        num_round = 0
        self.players = []
