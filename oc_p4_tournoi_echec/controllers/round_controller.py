...
class RoundController:
    ...
    def create_round(self, tournament):
        ...
        player_data = RoundController.read_players_tournament(player_list_file)
        ...
        if count == 1:
            RoundController.create_first_round(self, nm, list_players,
                                               round_list, tourn)
        else:
            # Récupération des résultats
            players_data_file = (
                "data/tournaments/" + tournament.get("id du tournoi"
                                                     ) + "_players.json"
            )
            players_data = []
            ...
            RoundController.create_match_conseq_players(
                sorted_players, rank_match,
                match_list, played_matches
            )
            # Si tous les matches n'ont pas été créés, on crée des matches
            # aléatoires pour les joueurs restants
            nbmatch = self.nbmatch
            RoundController.ramdom_match(
                match_list, nm, nbmatch, sorted_players,
                played_matches, round_list, rank_match, tournament
            )
    ...
    def update_round(self, tournament):
        ...
        update_player_tournament_path = ("data/tournaments/" + tournament.get("id du tournoi") + "_players.json")
        ...
        for number, match in enumerate(round_data):
            if match["id_round"] == id_selected and int(id_selected) <= len(round_data):
                RoundController.update_score_dictios(self, match, players_data)
                ...
                break
            elif match["id_round"] != id_selected and int(id_selected) <= len(round_data):
                pass
    ...
    def check_round(self, tournament):
        ...
        if all_played is True:
            ...
            tournaments = []
            # recuperation du contenu du fichier tournament_data
    ...
    def list_rounds(self, tournament):
        ...
        while iterate <= count:
            ...
            # afficher le round et la liste de match
            RoundView.display_rounds(self, self.name, file_path, round_data)
            iterate = iterate + 1

    def create_first_round(self, name, list_players, round_list, tournament):
        rank_match = 1
        ...
        file_path = "data/rounds/" + tournament.get("id du tournoi") + name + ".json"
        try:
    ...
    def read_players_tournament(player_list_file):
    ...
    def create_match_conseq_players(
        sorted_players,
        rank_match,
        match_list,
        played_matches
        ...
        for index_list_player, index_list_playertwo in zip(
            ...
            for number, adversary in enumerate(adversary_player2):
                if id_player1 == adversary[number]:
                    already_played = True
            # on crée un nouveau match Sils n'ont pas déjà joué ensemble,
            RoundController.create_match(
                already_played,
                sorted_players,
                rank_match,
                match_list,
    ...
    def update_score_dictios(self, match, players_data):
        ...
        # Mettre à jour les scores dans le dictio du match
        ...
        match["match"]["player1"]["adversaire"].append(match["match"]["player2"]["identifiant_national"])
        match["match"]["player2"]["classement"] = score2
        match["match"]["player2"]["adversaire"].append(match["match"]["player1"]["identifiant_national"])
        for player in players_data:
            if (player["identifiant_national"] == match["match"]["player1"]["identifiant_national"]):
                match_order = match["match"]["player1"]["classement"]
                player["classement"] = match_order
                player["adversaire"].append(match["match"]["player2"]["identifiant_national"])
            elif (
                player["identifiant_national"] == match["match"]["player2"]["identifiant_national"]
            ):
                match_order2 = match["match"]["player2"]["classement"]
                player["classement"] = match_order2
                player["adversaire"].append(match["match"]["player1"]["identifiant_national"])
